...
class CalendarTab extends React.PureComponent {
    ...
    render() {
        ...
        if (this.props.error) return <Retry className="calendar-retry" onClick={this.props.fetchData} />;
                    ...
                    {this.props.data.map(item => {
                        ...
                        return (
                            <li className={p('item')} key={item.id}>
                                <div className={p('time')}>{parseTime(new Date(item.localDateTime.replace(/-/g, '/')), '{h}:{i}')}</div>
